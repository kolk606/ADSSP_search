...
Select Case strAction
    Case "start"
         iRet = DoCmd("net start postgresql-x64-9.6")
    Case "stop"
         iRet = DoCmd("net stop postgresql-x64-9.6")
    Case "reload"
         iRet = DoCmd("""C:\Program Files\PostgreSQL\9.6\bin\pg_ctl.exe"" -D ""C:\Program Files\PostgreSQL\9.6\data"" reload")
    Case "restart"
	     iRet = DoCmd("net stop postgresql-x64-9.6")
		 If iRet = 0 Then
		     iRet = DoCmd("net start postgresql-x64-9.6")
